Happstack
HomeDownloadDocumentationCommunityBlog
happstack-lite
...
happstack-lite brings that simple, easy to use version of Happstack to light, without forcing you to give up any of the power and flexible.

To create happstack-lite, we have

Gathered all the essential types and functions you need to develop a web application into a single module Happstack.Lite so you don't have to hunt around for what you need.
...
But, here is the best part. happstack-lite is 100% compatible with Happstack. If you are developing an application using happstack-lite, and you need an advanced feature from Happstack, you can simple import the corresponding module and use it! To switch from happstack-lite to regular happstack simply change your import from Happstack.Lite to Happstack.Server and change serve to simpleHTTP. That's it!
...
To start the app we call the serve function. The first argument is an optional configuration parameter. The second argument is our web application:

> main :: IO ()
> main = serve Nothing myApp

A web application has the type ServerPart Response. You can think of ServerPart as the web equivalent of the IO monad.
...
When the user first views the form, the browser will request "/form" using the GET method. In the form tag, we see that the form will also be submitted to "/form" when the user presses the submit button. But in the form tag we have set the method attribute to POST.
...
In most web applications, we will want to serve static files from the disk such as images, stylesheets, javascript, etc. We can do that using the serveDirectory function.
...
In most cases, you don't want a user to upload a file just to have it deleted. Normally the upload handler would use moveFile or copyFile to move (or copy) the temporary file to a permanent location.

Powered by Happstack. Copyright 2012, SeeReason Partners LLC
